...
\begin{abstract}
We investigate the use of randomly generated data for the sake of pre-training a model, before seeing any domain-specific data. We justify this approach theoretically from the perspective of Kolmogorov complexity. We show empirically that randomly generated data, free of any domain assumptions---save the structural layout of the data---can be used to pretrain a model before it sees data, and that this approach several benefits. First, leads to better-than-chance zero-shot performance, that is, the model has some in-context test-set performance before seeing any training data. Second, when the model is finetuned on training data after pretraining, it converges faster and to better performance, even when the pretraining takes up part of the computational budget. And third, The resulting model shows better generalization both in-domain and out-of-domain. While these experiments are only small scale, we perform a scaling experiment that shows a promising trend.
