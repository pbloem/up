...
def go(
    ...
    for i in (bar := trange(batches)):
        ...
        with torch.no_grad():
            ...
            up.weights_init(cmp_source, init_mult_max=init_mult_max, mask_prob_max=mask_prob_max, mup=False)
        ...
        if accumulated >= mbraw: # perform a step
            ...
            # scale the gradients to average over the macrobatch (so far, pytorch has summed the gradients)
